...
class EncoderBlock(nn.Module):
    """Implements a single Transformer Encoder Block (Using Pre-LN)."""
    ...
    def forward(
        ...
        """
        Forward pass through the Encoder Block (Pre-LN variant).
        ...
        residual = x
        x_norm = self.norm1(x)
        attn_output_tuple = self.self_attention(
            query=x_norm, key=x_norm, value=x_norm, mask=mask
        )
        attn_output = attn_output_tuple[0]
        x = residual + self.dropout1(attn_output)  # Add residual *after* dropout

        # 2. Pre-LN Position-wise Feed-Forward + Residual
        residual = x
        x_norm = self.norm2(x)
