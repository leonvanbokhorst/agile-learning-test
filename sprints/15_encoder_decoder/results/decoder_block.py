...
class DecoderBlock(nn.Module):
    """Implements a single Transformer Decoder Block (Using Pre-LN)."""
    ...
    def forward(
        ...
        """
        Forward pass through the Decoder Block (Pre-LN variant).
        ...
        masked_attn_out_tuple = self.masked_self_attention(
            query=target_norm, key=target_norm, value=target_norm, mask=target_mask
        )
        masked_attn_out = masked_attn_out_tuple[0]
        target = residual + self.dropout1(masked_attn_out)

        # 2. Pre-LN Multi-Head Cross-Attention + Residual
        residual = target
        target_norm = self.norm2(target)  # Norm the output of the previous block
        # Note: encoder_output comes from the encoder, which should ideally also end with LN or be normed here.
        # Assuming encoder_output is appropriately scaled or the final encoder LN handles it.
        cross_attn_out_tuple = self.cross_attention(
            query=target_norm,
            key=encoder_output,  # Encoder output K/V don't get normed here
            value=encoder_output,
            mask=encoder_mask,
        )
        cross_attn_out = cross_attn_out_tuple[0]
        target = residual + self.dropout2(cross_attn_out)

        # 3. Pre-LN Position-wise Feed-Forward + Residual
        residual = target
        target_norm = self.norm3(target)
        ffn_output = self.feed_forward(target_norm)
        target = residual + self.dropout3(ffn_output)
